...
from aqi_functions_city import (
...
try:
    logging.info(f"Running city AQ pipeline at:{os.getcwd()}")

    data = fetch_city_data(
        city, start_date, end_date, limit, retries, csv_output_directory
    )
    ...
    city_predict_and_plot(
        model, cleaned_data, city, image_output_directory, num_prediction_days
    )
